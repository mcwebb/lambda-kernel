...
class Kernel {
   ...
   * @param {Transformer} [options.transformer]
   * @param {Middleware[]} [options.middlewares]
   * @param {boolean} [options.forceEnd]
   */
  constructor(options) {
    ...
    } else this.options = Object.assign({}, defaultOptions);

    if (!(this.options.transformer instanceof Transformer)) {
      throw new TypeError(
        'transformer provided to Kernel is not an ' +
        'instance of Transformer, it is: ' + typeof this.options.transformer
      );
    }
    ...
    if (!Array.isArray(this.options.middlewares)) {
      throw new TypeError(
        'middlewares provided to Kernel should be an array ' +
        'provided type is: ' + typeof this.options.middlewares
      );
    }
  ...
  dispatcher(event, context, cb, action, middlewares) {
    ...
    const env = {
      stage: stage,
      debug: ['PROD', 'PRODUCTION'].indexOf(stage) === -1
    };
